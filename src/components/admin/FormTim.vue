    ...
    <form @submit.prevent="handleSubmit" class="space-y-6">
        ...
        <div class="p-3 bg-gray-50 dark:bg-gray-700/30 rounded-lg border border-gray-100 dark:border-gray-600 flex items-center justify-between">
          ...
          <div class="flex items-center gap-3">
            ...
            <input 
              ...
              v-model="form.warna"
              :disabled="isSubmitting"
              class="w-10 h-10 cursor-pointer border-none rounded-lg shadow-sm p-0 bg-transparent overflow-hidden transition-transform hover:scale-110 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
            />
          </div>
        </div>
      </div>
        ...
        <div>
          ...
          <div class="relative group">
            ...
            <div 
              ...
              :class="[
                errors.ketuaTimId ? 'border-red-500' : 'border-gray-300 dark:border-gray-600',
                isSubmitting ? 'opacity-50 cursor-not-allowed bg-gray-100 dark:bg-gray-700' : ''
              ]"
              @click="!isSubmitting && (isLeaderDropdownOpen = !isLeaderDropdownOpen)"
            >
        ...
        <div>
          ...
          <div class="relative group">
            <div 
              ...
              :class="isSubmitting ? 'opacity-50 cursor-not-allowed bg-gray-100 dark:bg-gray-700' : ''"
              @click="!isSubmitting && (isOperatorDropdownOpen = !isOperatorDropdownOpen)"
            >
              ...
              <div v-else class="flex flex-wrap gap-2">
                <transition-group name="list">
                  ...
                  >
                    ...
                    <button type="button" @click.stop="!isSubmitting && removeOperator(operator.id)" class="ml-1.5 text-indigo-400 hover:text-indigo-600 focus:outline-none" :disabled="isSubmitting">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input 
            ...
            class="block w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm focus:ring-2 focus:ring-blue-500/50 outline-none disabled:bg-gray-100 disabled:text-gray-500 dark:disabled:bg-gray-700 dark:disabled:text-gray-400"
          />
      ...
      <div class="flex justify-end gap-3 pt-4 mt-6 border-t border-gray-200 dark:border-gray-700">
        ...
        >
          ...
          <span>{{ isEdit ? 'Simpan Perubahan' : 'Buat Tim' }}</span>
        </button>
      </div>
    </form>
  </div>
...
<script setup>
import { reactive, watchEffect, ref, computed, nextTick, watch } from 'vue';

const props = defineProps({
  initialData: { type: Object, default: null },
  userList: { type: Array, required: true },
  isEdit: { type: Boolean, default: false } // Pastikan properti ini diterima
});
...
const activeUsers = computed(() => {
  return props.userList.filter(user => user.isActive);
});
...
const filteredUsersForOperator = computed(() => {
  ...
  const availableUsers = activeUsers.value.filter(user => user.id !== form.ketuaTimId);
...
const handleSubmit = () => {
  ...
  if (validate()) {
    ...
    emit('submit', form);
    // isSubmitting will be reset when parent closes modal or validation fails
  }
};
</script>
