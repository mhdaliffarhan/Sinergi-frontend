<template>
  <div
    ...
    :class="{ 'h-screen overflow-hidden flex flex-col': mode === 'timeline' }"
  >
    ...
    <div class="max-w-7xl mx-auto pb-10 mt-6">
      ...
      <transition name="fade" mode="out-in">
        <div
          v-if="isLoading"
          class="flex flex-col items-center justify-center h-96 bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700"
        >
          <div class="relative w-16 h-16">
            <div
              class="absolute top-0 left-0 w-full h-full border-4 border-gray-100 dark:border-gray-700 rounded-full"
            ></div>
            <div
              class="absolute top-0 left-0 w-full h-full border-4 border-blue-500 rounded-full animate-spin border-t-transparent shadow-lg shadow-blue-500/20"
            ></div>
          </div>
          <p class="mt-4 text-gray-400 dark:text-gray-500 font-medium animate-pulse">
            Memuat data...
          </p>
        </div>
        ...
        <div v-else class="min-h-[500px]">
          ...
          <div v-if="(mode === 'team' || mode === 'person') && !isLoading">
            <KalenderFullCalendar :events="calendarEvents" @event-click="openModal" />
          </div>

          <!-- Child Component: Timeline (Mode Timeline) -->
          <div v-if="mode === 'timeline'" class="h-full">
            <KalenderTimeline
    ...
    <ModalAktivitas
      ...
      @close="closeModal"
      @go-to-detail="goToActivityDetail"
    />
  </div>
</template>
...
const prevMonth = () => {
  timelineCurrentDate.value = subMonths(timelineCurrentDate.value, 1)
}
const nextMonth = () => {
  timelineCurrentDate.value = addMonths(timelineCurrentDate.value, 1)
}
...
const fetchInitialData = async () => {
  ...
  try {
    ...
    allPegawai.value = usersResponse.data.items.sort((a, b) =>
      a.namaLengkap.localeCompare(b.namaLengkap),
    )

    // Cari User Kepala Kantor (berdasarkan jabatan string mengandung 'Kepala Kantor')
    kepalaKantorUser.value = allPegawai.value.find((p) =>
      p.jabatan?.namaJabatan?.toLowerCase().includes('kepala kantor'),
    )
...
const filteredPegawai = computed(() => {
  ...
  return allPegawai.value.filter(
    (p) => p.namaLengkap.toLowerCase().includes(query) || p.username.toLowerCase().includes(query),
  )
})
  ...
  filteredActivities.value.map((a) => {
    ...
    return {
      ...
      start: isAllDay ? a.tanggalMulai : `${a.tanggalMulai}T${a.jamMulai}`,
        ...
        : `${a.tanggalSelesai || a.tanggalMulai}T${a.jamSelesai}`,
      allDay: isAllDay,
