<template>
  <div
    class="min-h-screen font-sans transition-colors duration-300 relative overflow-hidden bg-gray-50 dark:bg-gray-900"
  >
    <!-- BACKGROUND DECORATIONS -->
    <div class="fixed inset-0 z-0 pointer-events-none overflow-hidden">
      <div
        class="absolute top-[-10%] left-[-10%] w-[600px] h-[600px] bg-blue-500/10 rounded-full blur-3xl opacity-60 dark:opacity-20"
      ></div>
      <div
        class="absolute bottom-[-10%] right-[-10%] w-[600px] h-[600px] bg-green-500/10 rounded-full blur-3xl opacity-60 dark:opacity-20"
      ></div>
    </div>
    ...
    <div class="relative z-10 max-w-7xl mx-auto">
      ...
      <div class="mb-8 sm:mb-10 flex flex-col md:flex-row md:justify-between gap-6">
        ...
        <div class="flex flex-col items-start gap-4 p-1 -m-1">
          <div>
            <h1
              class="text-2xl sm:text-4xl font-black text-gray-900 dark:text-white flex items-center gap-3"
            >
              <span class="text-3xl sm:text-5xl animate-bounce-slow">👋</span>
              <div class="flex flex-col">
                <span class="text-base sm:text-xl font-medium text-gray-500 dark:text-gray-400"
                  >Selamat {{ greeting }},</span
                >
                <span
                  class="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-green-600 leading-tight"
                >
                  {{ username }}
                </span>
              </div>
        ...
        <div class="flex flex-col items-start md:items-end gap-3">
          <!-- Tanggal -->
          <div
            class="flex items-center gap-2 bg-white/60 dark:bg-gray-800/60 backdrop-blur-md px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm"
          >
          ...
          <div class="flex flex-col md:items-end gap-2 w-full md:w-auto">
            <!-- Badge Role -->
            <div
              class="inline-flex items-center gap-2 px-3 py-1 bg-blue-50 dark:bg-blue-900/20 rounded-full border border-blue-100 dark:border-blue-800 self-start md:self-end"
            >
      ...
      <transition name="fade" mode="out-in">
        <div
          v-if="isLoading"
          class="flex flex-col items-center justify-center py-32 space-y-4 bg-white/50 dark:bg-gray-900/50 backdrop-blur-sm rounded-3xl border border-white/20"
        >
          <div class="relative w-16 h-16">
            <div
              class="absolute top-0 left-0 w-full h-full border-4 border-gray-200 dark:border-gray-700 rounded-full opacity-50"
            ></div>
            <div
              class="absolute top-0 left-0 w-full h-full border-4 border-blue-500 rounded-full border-t-transparent animate-spin"
            ></div>
          </div>
        ...
        <div v-else class="space-y-8 animate-fade-in-up">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <template v-else>
              ...
              <DashboardStats
                title="Total Tim"
                :value="authStore.user?.teams?.length || 0"
                icon="👥"
                color="green"
              />
            </template>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div class="lg:col-span-2 space-y-8">
              ...
              <div v-else class="space-y-8">
                ...
                <div class="h-[500px]">
                  <DashboardTodoList :items="todoList" :role="isKetuaTim ? 'leader' : 'member'" />
                </div>
              </div>
            </div>
...
const handleActivitySubmit = async (formData) => {
  ...
  try {
    ...
    const errorMsg =
      error.response?.data?.detail?.[0]?.msg || 'Gagal menyimpan. Periksa kembali isian Anda.'
