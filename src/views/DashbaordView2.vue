<template>
  <div class="container mx-auto p-4 sm:p-6 lg:p-8">
    ...
    <div v-else class="space-y-8">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        ...
        <button @click="isModalAktivitasOpen = true" class="mt-4 sm:mt-0 px-4 py-2 bg-blue-600 text-white rounded-lg shadow-md hover:bg-blue-700 transition-colors">
            + Buat Aktivitas
        </button>
      </div>
      ...
      <div v-if="isKepalaKantor">
        ...
        <section class="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-6 mt-8">
            <h2 class="text-xl font-semibold mb-4 text-gray-800 dark:text-gray-200">Tren Aktivitas Kantor (Per Bulan)</h2>
            <ProgressLineChart :series="activityTrendData.series" :categories="activityTrendData.categories" />
        </section>

        <section class="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-6 mt-8">
          <div class="flex items-center gap-4 mb-4 flex-wrap">
            <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-200">Jadwal Kantor</h2>
          </div>
      ...
      <div v-else-if="isKetuaTim">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
          <div class="flex items-center gap-3 w-full sm:w-auto">
            ...
            <select v-model="selectedTeamId" @change="fetchActivityTrend('team', selectedTeamId)" class="w-full sm:w-64 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm dark:bg-gray-700 dark:text-white focus:outline-none focus:ring-blue-500 focus:border-blue-500">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 mt-6">
            ...
            <DashboardCard icon="✅" title="Aktivitas Total Tim" :value="teamAktivitas?.length ?? 0" color="text-orange-500" />
            <DashboardCard icon="🗓️" title="Aktivitas Bulan Ini" :value="aktivitasBulanIni" color="text-yellow-500" />
        </div>

        <section class="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-6 mt-8">
            <h2 class="text-xl font-semibold mb-4 text-gray-800 dark:text-gray-200">Aktivitas Pemeriksaan Belum 100%</h2>
            <TabelProgressDokumen v-if="incompleteReviewActivities.length > 0" :dokumen-wajib="incompleteReviewActivities" @go-to-aktivitas="goToAktivitas" />
             <div v-else class="text-center text-gray-500 dark:text-gray-400 p-6">
                 Semua aktivitas tim sudah 100% dicek, atau tidak ada dokumen wajib.
            </div>
        </section>

        <section class="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-6 mt-8">
            <h2 class="text-xl font-semibold mb-4 text-gray-800 dark:text-gray-200">Tren Aktivitas Tim (Per Bulan)</h2>
            <ProgressLineChart :series="activityTrendData.series" :categories="activityTrendData.categories" />
        </section>

        <h2 class="text-2xl font-bold text-gray-900 dark:text-white pt-8 border-t border-gray-200 dark:border-gray-700">Tugas Pribadi Saya</h2>
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mt-6">
            <DashboardCard icon="📝" title="Total Aktivitas Total Saya" :value="totalAktivitasSaya" color="text-blue-500" />
            <DashboardCard icon="🗓️" title="Aktivitas Bulan Ini" :value="aktivitasBulanIni" color="text-yellow-500" />
      ...
      <div v-else>
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mt-6">
            <DashboardCard icon="📝" title="Aktivitas Total Saya" :value="totalAktivitasSaya" color="text-blue-500" />
            <DashboardCard icon="🗓️" title="Aktivitas Bulan Ini" :value="aktivitasBulanIni" color="text-yellow-500" />
            <DashboardCard icon="⏳" title="Dokumen Belum Selesai" :value="dokumenBelumSelesai" color="text-red-500" />
        </div>

        <section class="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-6 mt-8">
            <h2 class="text-xl font-semibold mb-4 text-gray-800 dark:text-gray-200">Tren Partisipasi Aktivitas (Per Bulan)</h2>
            <ProgressLineChart :series="activityTrendData.series" :categories="activityTrendData.categories" />
        </section>
        ...
        <section class="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-6 mt-8">
          <h2 class="text-xl font-semibold mb-4 text-gray-800 dark:text-gray-200">Jadwal Saya</h2>
          <div ref="calendar" class="w-full"></div>
        </section>
      </div>
    </div>
  ...
  <ModalWrapper :show="isModalAktivitasOpen" @close="isModalAktivitasOpen = false" title="Buat Aktivitas Baru">
    <FormAktivitas @close="isModalAktivitasOpen = false" @submit="handleActivitySubmit" tipe="Buat" :team-list="allTeams" :project-list="projectList" :pegawai-list="allUsers" :team-members="{}" />
  </ModalWrapper>
...
import { isWithinInterval, compareAsc, isFuture, isToday, format, parseISO, startOfMonth, endOfMonth, subMonths, differenceInMonths } from 'date-fns';
import { id } from 'date-fns/locale';
import timeGridPlugin from '@fullcalendar/timegrid';
import interactionPlugin from "@fullcalendar/interaction";

import DashboardCard from '@/components/dashboard/DashboardCard.vue';
import TabelProgressDokumen from '@/components/dashboard/TabelProgressDokumen.vue';
...
const activityTrendData = ref({ series: [], categories: [] });
...
const aktivitasBulanIni = computed(() => {
    const today = new Date();
    const startOfThisMonth = startOfMonth(today);
    const endOfThisMonth = endOfMonth(today);

    const sourceActivities = isKetuaTim.value ? teamAktivitas.value : allAktivitas.value;
    
    return sourceActivities.filter(aktivitas => {
        const tanggalMulai = aktivitas.tanggalMulai ? parseISO(aktivitas.tanggalMulai) : null;
        const tanggalSelesai = aktivitas.tanggalSelesai ? parseISO(aktivitas.tanggalSelesai) : tanggalMulai;
        
        if (!tanggalMulai) return false;

        const isOverlapping = (
            isWithinInterval(tanggalMulai, { start: startOfThisMonth, end: endOfThisMonth }) ||
            isWithinInterval(tanggalSelesai, { start: startOfThisMonth, end: endOfThisMonth }) ||
            (tanggalMulai < startOfThisMonth && tanggalSelesai > endOfThisMonth)
        );
...
// Sugesti 5: Tabel Pemeriksaan Dokumen Belum Lengkap
const incompleteReviewActivities = computed(() => {
    if (!isKetuaTim.value) return [];
    
    // Kembalikan hanya dokumen wajib yang belum dicek/disetujui
    return dokumenWajibTeam.value.filter(doc => !doc.statusPengecekan);
});


// FIX 3: Fungsi filterUpcomingAktivitas (Hilang sebelumnya)
const filterUpcomingAktivitas = () => {
    const today = new Date();
    upcomingAktivitasKantor.value = allAktivitasKepalaKantor.value.filter(aktivitas => {
        const tanggalMulai = parseISO(aktivitas.tanggalMulai);
        const tanggalSelesai = aktivitas.tanggalSelesai ? parseISO(aktivitas.tanggalSelesai) : tanggalMulai;
        return isFuture(tanggalMulai) || isFuture(tanggalSelesai) || isToday(tanggalMulai) || isToday(tanggalSelesai);
    }).sort((a, b) => {
        return compareAsc(parseISO(a.tanggalMulai), parseISO(b.tanggalMulai));
    });
};


// FIX 2: Fungsi fetchActivityTrend (Perbaikan Grafik Tren)
const fetchActivityTrend = async (groupType, groupId = null) => {
    const finalGroupId = groupId || (groupType === 'user' ? authStore.user.id : selectedTeamId.value);
    if (!finalGroupId) return;

    try {
        const today = new Date();
        const currentYear = today.getFullYear();
        const startOfJan = startOfMonth(new Date(currentYear, 0, 1));
        
        const monthsNeeded = differenceInMonths(today, startOfJan) + 1; 

        const response = await axios.get(`${baseURL}/api/aktivitas/trend`, {
            params: {
                group_by: groupType,
        ...
        for (let i = 0; i < monthsNeeded; i++) {
            const date = subMonths(today, monthsNeeded - 1 - i);
            const key = format(date, 'yyyy-MM');
            monthMap[key] = { label: format(date, 'MMM', { locale: id }), count: 0 };
            categories.push(key);
        }
        
        rawData.forEach(item => {
            if (monthMap[item.month_year]) {
                monthMap[item.month_year].count = item.activity_count;
            }
        });
        
        const sortedMonths = categories.sort(); 
        
        activityTrendData.value = {
            categories: sortedMonths.map(key => monthMap[key].label),
            series: [{
                name: groupType === 'user' ? 'Keikutsertaan' : 'Jumlah Aktivitas',
                data: sortedMonths.map(key => monthMap[key].count)
            }]
        };

    } catch (error) {
        console.error("Gagal memuat data trend aktivitas:", error);
        activityTrendData.value = { series: [], categories: [] };
    }
};
...
const fetchDashboardData = async () => {
    ...
    try {
        ...
        if (!user) { router.push({ name: 'login' }); return; }
        ...
        projectList.value = projectsRes.data.items.map(p => ({ id: p.id, namaProject: p.namaProject, teamId: p.teamId, projectLeaderId: p.projectLeaderId }));
        ...
        if (isKepalaKantor.value) {
            ...
            await fetchActivityTrend('user', user.id); 
            filterUpcomingAktivitas();

        } else if (isKetuaTim.value) {
            const teamId = selectedTeamId.value;
            if (teamId) {
                const [aktivitasRes, teamDetailsRes, dokumenTeamRes] = await Promise.all([
                ...
                teamAktivitas.value = aktivitasRes.data; 
                team.value = teamDetailsRes.data;
                dokumenWajibTeam.value = dokumenTeamRes.data; 
                
                const [aktivitasSayaRes, dokumenSayaRes] = await Promise.all([
                    axios.get(`${baseURL}/api/users/${user.id}/aktivitas`),
                    axios.get(`${baseURL}/api/users/${user.id}/dokumen-wajib`)
                ]);
                allAktivitas.value = aktivitasSayaRes.data;
                dokumenWajibSaya.value = dokumenSayaRes.data;

                await fetchActivityTrend('team', teamId);
            }
        } else { // Anggota Tim
...
const handleActivitySubmit = async (formData) => {
    ...
    try {
        ...
        isModalAktivitasOpen.value = false;
...
// FIX 4: Fungsi formatPeriode (Hilang sebelumnya)
...
// FIX 5: Fungsi renderCalendar (Hilang sebelumnya)
const renderCalendar = () => {
    if (!calendar.value) return;
    if (fullCalendarInstance) {
        fullCalendarInstance.destroy();
    }
    const events = calendarEvents.value.map(a => {
        const endDate = a.tanggalSelesai ? parseISO(a.tanggalSelesai) : parseISO(a.tanggalMulai);
...
watch(() => authStore.user?.ketuaTimAktif, (newVal) => {
    if (newVal && newVal.length > 0 && !selectedTeamId.value) {
        selectedTeamId.value = newVal[0].id;
    }
}, { immediate: true });

watch(selectedTeamId, (newId) => {
    if (isKetuaTim.value && newId) {
        fetchDashboardData();
    }
});

watch([allAktivitas, teamAktivitas], () => { 
